...
def submit_action(file_data):
    ...
    result = mrz(file_data)  # must return 4 values
    end_time = time.time()
...
if st.button("Submit"):
    if file_data is None:
        ...
        # expect only 4 values from mrz()
        [mrz_preprocessed, mrzText, PID, family_name], execution_time = submit_action(file_data)
        ...
        if mrzText is None:
            ...
            data = {
                ...
                # "Given Names": given_names,  # remove or handle later
            }
            ...
            # rewind uploader if mrz() consumed it
            file_data.seek(0)
